...
In object-oriented programming, a class can be derived from some _ancestor_ class and thus _inherit_ its properties and methods. Moreover, several classes can be derived from a single ancestor producing a mix of unique and shared functionality. This means that instead of rewriting the same code for each class, you can define a common code in an ancestor class and focus on differences or additional methods and properties in descendants.
...
There are other ways of achieving common behavior (generalization) in Python without orderly inheritance, such as "duck typing"^[Yes, it is really called "duck typing".] or mixins but this will be a topic of another chapter.
...
::: {.practice}
Copy-paste and test [Accumulator]{.program-function} class code in a Jupiter notebook.
:::
...
::: {.practice}
Add [subtract]{.program-function} method to the [FlexibleAccumulator]{.program-function} class in a Jupiter notebook. Add testing.
:::
...
::: {.practice}
Create [SuperFlexibleAccumulator]{.program-function} class and define super flexible  [add]{.program-function} and [subtract]{.program-function} methods that have [value]{.program-function} parameter ( in a Jupiter notebook). Test them!
:::
...
::: {.practice}
Add [initial_value] parameter to the constructor of the [SuperFlexibleAccumulator]{.program-function} class in a Jupiter notebook. Test it!
:::
...
::: {.practice}
Modify [subtract()]{.program-function} method of [SuperFlexibleAccumulator]{.program-function} to utilize [add()]{.program-function} in a Jupiter notebook. Test it!
:::
...
::: {.program}
Put [FlappyBird]{.program-function} class code into a separate file. Use it instead of [ImageStim]{.program-function} in [code02.py]{.program-filename}.
:::
...
::: {.program}
Add bird size setting. Use it in [FlappyBird]{.program-function} class constructor.
:::
...
Now you need to call the `update_per_frame()` on _each_ frame before drawing the bird. This should make your bird fall of the screen! (Experiment with `"Initial vertical speed"` setting to make it fall faster or slower or even upwards!)

::: {.program}
Update [FlappyBird]{.program-function} class. Use [update_per_frame]{.program-function} method in [code03.py]{.program-filename}.
:::

## Timing the fall
Currently, the speed of our bird's fall is measured in norm units per _frame_. This works but these are not the most convenient units to think in. Plus, it relies on PsychoPy (and the rest of our code) to ensure that time between individual frames is exactly the same. This is _mostly_ the case and an occasionally slow bird is not a big problem for a game. However, that might be a problem for an actual experiment that requires precise timing of movement. Thus, we need to think about vertical speed in units of norm units per _second_ and measure time between calls ourselves.

Create a new [Clock](https://psychopy.org/api/clock.html#psychopy.clock.Clock) attribute that will count the time elapsed since it last reset (I would call it `frame_timer`). Create an `update()` method to compute $y_{new} = y + vspeed * T_{elapsed}$, where $T_{elapsed}$ is the time elapsed between frames. Do not forget to reset the timer! (What will happen if you do forget?) 
...
::: {.program}
Update [FlappyBird]{.program-function} class with a timer and [update]{.progam-function} method. Use [update]{.progam-function} instead of [update_per_frame]{.progam-function} in [code04.py]{.program-filename}.
:::
...
::: {.program}
Update [FlappyBird]{.program-function} class with acceleration due to gravity.
:::
...
::: {.program}
Add [flap]{.program-function} method to [FlappyBird]{.program-function} class. Use it in [code04.py]{.program-filename} whenever player presses _space_.
:::
...
::: {.program}
Add [is_airborne]{.program-function} method to [FlappyBird]{.program-function} class. Use it in [code06.py]{.program-filename} as an additional condition for the game loop.
:::
...
::: {.program}
Turn [is_airborne]{.program-function} into a property. Use it as property in [code07.py]{.program-filename}.
:::
...
::: {.program}
Create [Obstacle]{.program-function} class in a separate file. Use it in [code08.py]{.program-filename}.
:::
...
::: {.program}
Add [update]{.program-function} method to [Obstacle]{.program-function} class. Use it in [code09.py]{.program-filename}.
:::
...
::: {.program}
Add [check_if_hit]{.program-function} method to [Obstacle]{.program-function} class. Use it in [code10.py]{.program-filename}.
:::
...
::: {.program}
Create [ObstaclesManager]{.program-function} class. Use it in [code11.py]{.program-function}.
:::
...
::: {.program}
Update [update]{.program-function} of the [ObstaclesManager]{.program-function} class.
:::
...
::: {.program}
Update your code and use it in [code12.py]{.program-filename}.
:::
