...
To implement this program, you will need to learn about functions, how to document them, and how to use your own libraries. Grab the [exercise notebook](notebooks/04-Guess the number - AI.ipynb) before we start!
...
:::{.program}
Update your code in [code02.py]{.program-filename}.
:::
...
::: {.program}
Put your code into [code03.py]{.program-filename}.
:::

## Splitting interval in the middle {#round-function}
...
:::{.program}
Put you **split_interval()** function and the testing code into [code04.py]{.program-filename}.
:::
...
:::{.program}
Put both functions and the script code into [code05.py]{.program-filename}.
:::
...
:::{.program}
Put the updated code into [code06.py]{.program-filename}.
:::
...
:::{.program}
Put the updated code into [code07.py]{.program-filename}.
:::
...
:::{.program}
Put the updated code into [code08.py]{.program-filename}.
:::

## Using you own libraries
You already know how to [use existing libraries](#using-libraries) but you can also create and use your own. Take the two functions that you developed and put them into a new file called [utils.py]{.filename} (do not forget to put a multiline comment at the top of the file to remind you what is inside!) . Copy the remaining code (the global script) into [code09.py]{.filename}. It will not work in its current state as it won't find the two functions (try it to see the error message), so you need to import from your own [utils]{.filename} module. Importing works exactly the same way as for other libraries. Note that even though your file is [utils.py]{.filename}, the module name is [utils]{.filename} (without the extension).

:::{.program}
Put function into [utils.py]{.program-filename}, the remaining code into [code09.py]{.program-filename}.
:::

## Ordnung muss sein!{#keep-imports-tidy}
So far, you only imported one library at most. However, as Python is highly modular, it is very common to have many imports in a single file. There are several rules that make it easier to track the imports. When you import libraries, all import statements should be at the top of your file and you should avoid putting them in random order. The recommended order is 1) system libraries, like [os]{.filename} or [random]{.filename}; 2) third-party libraries, like ]psychopy]{.filename}; 3) your project modules. And, within each section you should put the libraries _alphabetically_, so
```python
import os
import random
```
