...
Bevor du anfängst, erstelle einen neuen Ordner (innerhalb deines [Kursordners](#files-folder)) namens "guess-the-number" zum Beispiel, lade das [Übungsheft](notebooks/Guess the number - single round.ipynb) herunter, kopiere es in den neu erstellten Ordner und öffne es in [Jupyter Notebook](#jupyter-notebooks). Wie im Kapitel zuvor wird es Übungen zum Lesen und Verständnis des Codes enthalten.
